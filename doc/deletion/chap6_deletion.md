...
```bash
BACKEND_SERVICE_NAME=$(gcloud compute backend-services list --format=json | jq -r .[].name | grep ${SERVICE_PREFIX})
gcloud compute backend-services update $BACKEND_SERVICE_NAME --global --security-policy=
gcloud compute security-policies delete ${SERVICE_PREFIX}-waf-policy --quiet
```
...
gcloud compute target-https-proxies update ${SERVICE_PREFIX}-https-proxies --ssl-certificates=${CNSRUN_CERT}
```

続いて、GoogleマネージドSSL証明書の削除をします。

```bash
gcloud compute ssl-certificates delete ${SERVICE_PREFIX}-frontend --quiet
```
...
```bash
gcloud compute target-https-proxies update ${SERVICE_PREFIX}-https-proxies --clear-ssl-policy
gcloud compute ssl-policies delete ${SERVICE_PREFIX}-ssl-policy --global --quiet
```
...
```bash
gcloud beta container binauthz attestors delete ${SERVICE_PREFIX}-attestor
```
